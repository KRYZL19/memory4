...
function processActivityObject(msg, done) {
    if (!validateActivityObject(msg)) {
        return done(new Error(`activity-object schema validation failed: ${tv4_1.default.error.dataPath} = ${tv4_1.default.error.message}`));
    }
    msg.displayName = ensureDisplayName(msg);
    return done(msg); // passed validation, on to next handler in middleware chain
}
function processActivityStream(msg, done) {
    let stream;
    ...
    if (!validateActivityStream(msg)) {
        return done(new Error(`actvity-stream schema validation failed: ${tv4_1.default.error.dataPath}: ${tv4_1.default.error.message}`));
    }
    // passed validation, on to next handler in middleware chain
    return done(expandStream(msg));
}
function processCredentials(msg, done) {
    ...
    if (!validateCredentials(msg, credentialsSchema)) {
        return done(new Error(`credentials schema validation failed: ${tv4_1.default.error.dataPath} = ${tv4_1.default.error.message}`));
    }
    // passed validation, on to next handler in middleware chain
    return done(expandStream(msg));
}
...
function validate(type, sockethubId) {
    const sessionLog = (0, debug_1.default)(`sockethub:validate:${sockethubId}`);
    ...
    return (msg, done) => {
        ...
        if (!ensureObject(msg)) {
            ...
            done(new Error(`platform context ${msg.context} not registered with this sockethub instance.`));
        }
